...
public class Bomb extends MovingEntity {
    ...
    protected UnusualBoom[] _explosions = null;
    ...
    protected void explosion() {
        ...
        _explosions = new UnusualBoom[4];

        for (int i = 0; i < _explosions.length; i++) {
            _explosions[i] = new UnusualBoom((int)_x, (int)_y, i, GameLoop.getBombRadius(), _board);
        }
    }

    public Boom explosionAt(int x, int y) {
        if(!_exploded) return null;

        for (int i = 0; i < _explosions.length; i++) {
            if(_explosions[i] == null) return null;
            Boom e = _explosions[i].explosionAt(x, y);
            if(e != null) return e;
        }
    ...
    public boolean collide(Entity e) {
        ...
        if(e instanceof Player) {
            double diffX = e.getX() - Convert.tileToPixel(getX());
            double diffY = e.getY() - Convert.tileToPixel(getY());
